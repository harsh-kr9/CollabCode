...
const Signin = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setIsButtonDisabled(true); // Disable button to prevent double-submit
    firebase.signInUserWithEmailAndPassword(email, password)
      .then((userCredential) => {
  ...
  const signinwithgoogle = async () => {
    try {
      const result = await firebase.signInUserWithGoogle();
      const user = result.user;
      console.log(user);
      dispatch(setUsername(user.displayName || user.email.split('@')[0])); // Dispatch the username
      await axiosInstance.post('/api/v1/auth/register', { name: user.displayName, email: user.email, uid: user.uid });
      navigate('/dashboard'); // Redirect to home page
    } catch (error) {
