...
const javacPath = process.platform === 'win32'
    ? 'javac'
    : (fs.existsSync(path.join(__dirname, '..', 'jdk', 'bin', 'javac'))
        ? path.join(__dirname, '..', 'jdk', 'bin', 'javac')
        : 'javac');

const javaPath = process.platform === 'win32'
    ? 'java'
    : (fs.existsSync(path.join(__dirname, '..', 'jdk', 'bin', 'java'))
        ? path.join(__dirname, '..', 'jdk', 'bin', 'java')
        : 'java');
    ...
    try {
        ...
        if (lang === 'python') {
            ...
            fs.writeFile(tempFilePath, code, (err) => {
                ...
                // Compile and run the Java code using dynamic portable JDK paths
                exec(`"${javacPath}" ${tempFilePath} && "${javaPath}" -classpath ${folderPath} ${className}`, (error, stdout, stderr) => {
                    if (error) {
                        res.status(500).send({data:error});
                        return;
                    }
            ...
            tempFilePath = path.join(folderPath, 'temp.js');
            fs.writeFile(tempFilePath, code, (err) => {
                if (err) {
                    console.error('Error writing code to file:', err);
                    res.status(500).send('Error writing code to file');
