...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsButtonDisabled(true); // Disable the button immediately to prevent double submission
    ...
    try {
      ...
      if (user) {
        try {
          ...
          dispatch(setReduxUsername(username)); // Dispatch the updated username to Redux store
          ...
          console.error('Backend registration or profile update failed. Rolling back Firebase user:', backendError.message);
          // Rollback: delete the created user from Firebase Auth
          try {
            await user.delete();
          } catch (deleteError) {
            console.error('Failed to delete user during rollback:', deleteError.message);
          }
          throw backendError;
        }
      }
